...
exports.getProfile = async (req, res) => {
  try {
    let identifier;
    const { identifier: usernameOrId } = req.params;

    if (mongoose.Types.ObjectId.isValid(usernameOrId)) {
      identifier = { _id: usernameOrId };
    } else {
      identifier = { username: usernameOrId };
    }

    const user = await User.findById(req.user.id);
    const profile = await User.findOne(identifier).select("-password").populate("friends","first_name last_name username picture");
    const friendship = {
      friends: false,
      following: false,
      requestSent: false,
